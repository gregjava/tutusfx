...
<html>
	<head>
		...
		<link rel="icon" href="../../assets/images/logo_bar.png" type="image/x-icon" />
		<link rel="shortcut icon" href="../../assets/images/logo_bar-81.png" type="image/x-icon" />
		...
		<style>
			...
			@-webkit-keyframes pixAnimation {
				...
				100% {left:0px; top:0px; background:url("../../assets/images/forex_trading_15.png") no-repeat center; -webkit-background-size: cover; background-size: cover;}
			}
			...
			@-moz-keyframes pixAnimation {
				...
				100% {left:0px; top:0px; background:url("../../assets/images/forex_trading_15.png") no-repeat center; -moz-background-size: cover; background-size: cover;}
			}
			...
			@-o-keyframes pixAnimation {
				...
				100% {left:0px; top:0px; background:url("../../assets/images/forex_trading_15.png") no-repeat center; -o-background-size: cover; background-size: cover;}
			}
			...
			@keyframes pixAnimation {
				...
				100% {left:0px; top:0px; background:url("../../assets/images/forex_trading_15.png") no-repeat center; background-size: cover;}
			}
	...
	<body>
		<?php include '../../assets/db/signup.php'; ?>
		<script type="text/javascript">
		  ...
		  <div class="maincolumn">
			<div class="card" id="login-form" >
				<form method="post" name="signupform" action="../login.html" onsubmit="return checkData()" autocomplete="on" >
					...
					<div class="form-group">
						<div class="input-group"> <span id="posdatespan" class="input-group-addon">Date of Position: </span>
					...
					<div class="form-group">
						<div class="input-group"> <span id="jobdatespan" class="input-group-addon">Employment Date: </span>
					...
					<div class="form-group">
						<div class="input-group">
							<span class="input-group-addon" id="togglepassview"><span id="pswdImg" class="glyphicon glyphicon-lock"></span></span>
							<input type="password" name="pwd" id="pwd" class="form-control masked" placeholder="Please copy password to a safe location" maxlength="64" required/>
					...
					<div class="form-group contactUs-regardingSelect">
						...
						<form method="post" name="country_form" action="checkCountry()" autocomplete="on">
							<select name="country" id="country" class="form-control" required>
								<option value="">Select Country</option>
								<?php include_once '../../assets/db/config.php'; include_once '../../assets/db/signup.php';
					...
					<div class="form-group contactUs-regardingSelect">
						<div class="input-group"><span class="input-group-addon">Sub-Region: </span>
							<form method="post" name="state_form" action="checkState()" autocomplete="on">
								<select id="state" name="state" class="form-control" required>
									<option value="">Select State or Sub-Region</option>
									<?php include_once '../../assets/db/config.php'; include_once '../../assets/db/signup.php';
										if (isset($_POST['country']) ? $_POST['country'] : false) { $country = $_POST['country']; $_SESSION['countryID'] = $country; } 
										else { echo "Tutusfx was not able to determine your country"; exit; }
										$result = $user->runQuery("SELECT stateName, stateID FROM tutusfxc_Geo.states WHERE countryID=".$country);
										$result->execute(); $record = $result->fetch(PDO::FETCH_ASSOC);
										try { foreach ( $record as $value ) { echo "<option value=".($value['stateID']).">".($value['stateName'])."</option>"; } } 
										catch(PDOException $e){ echo "Problem encountered applying PDO: ".$e->getMessage(); }
									?>
								</select><span class="text-danger"></span>
							</form>
					...
					<div class="form-group contactUs-regardingSelect">
						...
						<select name="city" id="city" class="form-control" required>
							...
							<?php include '../../assets/db/config.php'; include '../../assets/db/signup.php';
								if (isset($_POST['state']) ? $_POST['state'] : false) { $state = $_POST['state']; $_SESSION['state'] = $state; } 
								else { echo "Tutusfx was not able to determine your state"; exit; }
								$result = $user->runQuery("SELECT cityName FROM tutusfxc_Geo.cities WHERE stateID=".$state);
								$result->execute(); $record = $result->fetch(PDO::FETCH_ASSOC);
								try { foreach ( $record as $value ) { echo "<option value=".($value['cityID']).">".($value['cityName'])."</option>"; } } 
								catch(PDOException $e){ echo "Problem encountered applying PDO: ".$e->getMessage(); }
							?>
					...
					<script>
						function checkData(){
							var x = document.forms["signupform"]["acct_name"].value, x1 = document.forms["signupform"]["fname"].value, x2 = document.forms["signupform"]["lname"].value,
								...
								x18 = document.forms["signupform"]["email"].value, x19 = document.forms["signupform"]["pm"].value;
							if (x=="") { 
		...
		<?php
			if (!$error) {
				if ( isset($acct_name) && $_SESSION['userSession']==="" ) $_SESSION['userSession'] = $acct_name;
				...
				header("Location: https://www.tutusfx.com/redirects/login.html");
				exit;
			} else {
		...
		<script>
			...
			var footertexts = document.getElementById("footerDivTexts"); var ads = document.getElementById("ads");  
			var pswdImg = document.getElementById("pswdImg"); 
			var footerclosebtn = document.getElementById("close"); var upperclosebtn = document.getElementById("upperclosebtn"); 
			var footerclosing = true; var upperclosing = false;
			
			function doclose() {
				if (footerclosing==true) {
					footerdiv.style.height = "0px"; footerdiv.style.float = "bottom"; footerdiv.style.background = "none";
					footerclosebtn.style.backgroundColor = "#002100"; footerclosebtn.style.color = "white"; footerclosebtn.classList.remove("glyphicon-arrow-up"); footerclosebtn.classList.add("glyphicon-arrow-down");
					footerheader.style.fontSize = "0%"; footertexts.style.fontSize = "0%"; footerclosebtn.style.visibility = "none"; footerclosing = false;
				} else {
					footerdiv.style.height = "30%"; footerdiv.style.top = "70%"; footerdiv.style.backgroundColor = "#002100"; footerdiv.style.zIndex = "1000";
					footerclosebtn.style.backgroundColor = "white"; footerclosebtn.style.color = "#002100"; footerclosebtn.classList.remove("glyphicon-arrow-down"); footerclosebtn.classList.add("glyphicon-arrow-up");
					footerheader.style.fontSize = "100%"; footertexts.style.fontSize = "100%"; footerclosing = true;
				}
			}
			
			function doupperclosing() { 
				if (upperclosing==true) { rates.classList.remove("upperbar");
					rates.style.height = "0px"; rates.style.background = "none"; 
					upperclosebtn.classList.remove("glyphicon-arrow-right"); upperclosebtn.classList.add("glyphicon-arrow-left");
					upperclosebtn.classList.remove("close-upper"); upperclosebtn.classList.add("open-upper");
					ads.style.fontSize = "0%"; upperclosing = false;
				} else { rates.classList.add("upperbar");
					rates.style.height = "50px"; rates.style.backgroundColor = "#002100";
					upperclosebtn.classList.remove("glyphicon-arrow-left"); upperclosebtn.classList.add("glyphicon-arrow-right");
					upperclosebtn.classList.remove("open-upper"); upperclosebtn.classList.add("close-upper");
					ads.style.fontSize = "100%"; upperclosing = true;
				} myFunction();
			} myFunction();
			...
			function myFunction() { 
				...
				if (upperclosing==true) { rates.classList.remove("upperbar");
					rates.style.height = "0px"; rates.style.float = "top"; rates.style.background = "none"; 
					...
					rates.style.height = "50px"; rates.style.top = "0px"; rates.style.backgroundColor = "#002100"; rates.style.zIndex = "1000";
					upperclosebtn.style.background = "white"; upperclosebtn.style.color = "#002100"; upperclosebtn.classList.remove("glyphicon-arrow-left"); upperclosebtn.classList.add("glyphicon-arrow-right");
			...
			function checkState(){
				var val = document.forms["state_form"]["state"].value;
				if (val!="") { // a value has been entered
				  var s = document.createElement("option");
				  s.innerHTML = "jsonreflector.php?y=" + JSON.stringify(val);
				  document.getElementById("city").appendChild(s);
				}
			}

			function myFunc2(myObj) { var y, txt = ""; for (y in myObj) { txt += myObj[y].cityName + "<br>"; } alert(txt); }
			
			function checkCountry(){
				var val = document.forms["country_form"]["country"].value;
				if (val!="") { // a value has been entered
				  var s = document.createElement("option");
				  s.innerHTML = "jsonreflector.php?x=" + JSON.stringify(val);
				  document.getElementById("state").appendChild(s);
				}
			}

			function myFunc(myObj) { var x, txt = ""; for (x in myObj) { txt += myObj[x].stateName + "<br>"; } alert(txt); }
			...
			function show() { var p = document.getElementById('pwd'); p.setAttribute('type', 'text'); pswdImg.classList.remove("glyphicon-lock"); pswdImg.classList.add("glyphicon-open");}

			function hide() { var p = document.getElementById('pwd'); p.setAttribute('type', 'password'); pswdImg.classList.remove("glyphicon-open"); pswdImg.classList.add("glyphicon-lock");}

			var pwShown = 0; document.getElementById("togglepassview").addEventListener("click", function () { if (pwShown == 0) { pwShown = 1; show(); } else { pwShown = 0; hide(); } }, false);
