<?php
	...
	if ( (isset($_SESSION['user']) && $_SESSION['user']==="Admin") ) {
		...
		if( isset($_POST['btn-login']) ) {
			...
			} else if ( !(strpos($email,".com")>0 && strpos($email,"@")>0) ) { // address does not end with .com
			...
			if (!$error) { // check if provided email exists or not
				try {
					if ($user->doLogin($email,$email,$pass)) {
						$_SESSION['errTyp'] = "Welcome";
						$_SESSION['errMSG'] = " ".$_SESSION['user'] = $email;
						...
						$_SESSION['errTyp'] = "ERROR";
						$_SESSION['errMSG'] = "Login failed, try again later...";
