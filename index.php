<head>
	...
	<meta property="og:description" content="Tutusfx is an incentivized Forex/CFDs/Cryptocurrency trading platform built on blockchain technology. Trade Forex/CFDs/Cryptocurrency on our advanced, web-based trading platform designed with you, the user, in mind."/>
	...
	<link rel="alternate" hreflang="ru-RU" href="ru/index.html"/>
	<link rel="alternate" hreflang="ar-AR" href="ar/index.html"/>
	<link rel="alternate" hreflang="sv-SE" href="sv/index.html"/>
	<link rel="alternate" hreflang="nl-NL" href="nl/index.html"/>
	<link rel="alternate" hreflang="nb-NO" href="nb/index.html"/>
	<link rel="alternate" hreflang="pl-PL" href="pl/index.html"/>
	...
	<div class="container">
	  <div class="leftColumn">
		<div class="card" id="app">
		  <h3>Top Trades</h3><hr/>
		  <p>
			<!-- Now we provide each top-trade with the tradeprop object it's representing, so that its content can be dynamic. We also need to provide each component with a "key", which will be explained later. -->
			<top-trade v-for="item in tradeData" v-bind:tradeprop="item" v-bind:key="item.id"></top-trade><br/>
		  </p>
		  <script>
			Vue.component('top-trade', { 
				props: ['tradeprop'], 
				template: '<div class="top-trade fakeimg trendingtrades"><p class="trendingtradeusername"> {{ tradeprop.currency+";" }}</p><p>{{ "Current Profit: "+tradeprop.profit+";"+"  Profit Margin: "+tradeprop.margin+";" }}</p><p>{{ "Trade Volume: "+tradeprop.volume+";"+"  Token Charges: "+tradeprop.charges+";" }}</p><p v-html="tradeprop.user"></p><hr/></div>' 
			})
		...
		<div class="card" id="app2">
		  ...
		  <p>
			<!-- Now we provide each top-trade with the tradeprop object it's representing, so that its content can be dynamic. We also need to provide each component with a "key", which will be explained later. -->
			<top-witnesses v-for="witness in witnesses" v-bind:witnessprop="witness" v-bind:key="witness.id"></top-trade><br/>
		  </p>
		  <p class="rightAlign"><a href="">More...</a></p><br/><p><hr/>
		  <script>
			Vue.component('top-witnesses', { 
				props: ['witnessprop'], 
				template: '<p> <span v-html="witnessprop.id+1"></span>&nbsp;<span v-html="witnessprop.name"></span></p>' 
			})
			...
			var app = new Vue({
			  ...
			  data: {
				witnesses: [
				  { id: 0, name: '<a href="witness/@darkspot">@darkspot</a>' },
				  { id: 1, name: '<a href="witness/@kutygee">@kutygee</a>' },
				  { id: 2, name: '<a href="witness/@thegenius">@thegenius</a>' },
				  { id: 3, name: '<a href="witness/@yogolod">@yogolod</a>' },
				  { id: 4, name: '<a href="witness/@ynl">@ynl</a>' },
				  { id: 5, name: '<a href="witness/@javakid">@javakid</a>' },
				  { id: 6, name: '<a href="witness/@icey">@icey</a>' },
				  { id: 7, name: '<a href="witness/@barbie12">@barbie12</a>' },
				  { id: 8, name: '<a href="witness/@ultra1">@ultra1</a>' },
				  { id: 9, name: '<a href="witness/@futile">@futile</a>' }
				]
			  }
			})
		  </script>
	  ...
	  <div class="rightColumn">
		<div class="card" id="app3">
		  <h3>Blockchain Posts</h3><hr/>
		  <p>
			<blog-post v-for="post in posts" v-bind:blockchainpost="post" v-bind:key="post.id"></blog-post>
		  </p>
		  <script>
			Vue.component('blog-post', { 
				props: ['blockchainpost'], 
				template: '<div class="blog-post"><h3>{{ blockchainpost.title }}</h3><hr/><h5><b> &nbsp; {{ blockchainpost.desc }}</b>, {{ blockchainpost.date }}</h5><div class="fakeimg largepicture">{{ blockchainpost.image }}</div><hr/><div v-html="blockchainpost.content"></div></div>'
			})
			...
			var app = new Vue({
			  ...
			  data: {
				posts: [
				  { id: 0, title: 'Post 2', desc: 'Title description', date: 'Nov 22, 2019', image: 'Image 2', content: '<p>Some text from Post 2...</p><p>Sunt in culpa qui officia deserunt mollit anim id est laborum consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p><hr/>'},
				  { id: 1, title: 'Post 1', desc: 'Title description', date: 'Nov 15, 2019', image: 'Image 1', content: '<p>Some text from Post 1...</p><p>Sunt in culpa qui officia deserunt mollit anim id est laborum consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco.</p><hr/>'}
				]
			  }
			})
		  </script>
